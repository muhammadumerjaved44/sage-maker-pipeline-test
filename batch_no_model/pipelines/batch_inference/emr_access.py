...
#        self.client.delete_application(applicationId=self.application_id)
        ...
        response = self.client.start_job_run(
            ...
            jobDriver={
                "sparkSubmit": {
                    ...
                    "sparkSubmitParameters": "--conf spark.executor.cores=1 --conf spark.cores.max=5 --conf spark.executor.memory=4g --conf spark.driver.cores=1 --conf spark.driver.memory=4g --conf spark.executor.instances=1",
                }
            },
        )
    ...
    def fetch_driver_log(
        ...
        file_location = f"{self.s3_log_prefix}/applications/{application_id}/jobs/{job_run_id}/SPARK_DRIVER/{log_type}.gz"
        try:
...
def parse_args():
    ...
    required_named.add_argument(
        "--application-id",
        help="EMR Serverless running application id",
        required=True,
    )
    required_named.add_argument(
        "--script-location",
        help="Location of script to run as EMR job",
        required=True,
    )
    required_named.add_argument(
        "--input-table",
        help="Location of input data to be passed as argument to python script",
        required=True,
    )
    required_named.add_argument(
        "--output-table",
        help="Location of output data to be passed as argument to python script",
        required=True,
    )
